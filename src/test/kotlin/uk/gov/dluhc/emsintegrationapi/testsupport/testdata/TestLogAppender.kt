...
class TestLogAppender : AppenderBase<ILoggingEvent>() {
    ...
    companion object {
        ...
        fun hasLog(
            message: String,
            level: Level,
        ): Boolean = logList.firstOrNull { hasMessage(it, message, level) } != null

        fun hasNoLogMatchingRegex(
            pattern: String,
            level: Level,
        ): Boolean = logList.firstOrNull { hasMessageMatchingRegex(it, Regex(pattern), level) } == null

        fun getLogEventMatchingRegex(
            pattern: String,
            level: Level,
        ): ILoggingEvent? = logList.firstOrNull { hasMessageMatchingRegex(it, Regex(pattern), level) }
        ...
        private fun hasMessageMatchingRegex(
            event: ILoggingEvent,
            regex: Regex,
            level: Level,
        ): Boolean {
