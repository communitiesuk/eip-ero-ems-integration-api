...
internal class DeletePostalVoteApplicationsIntegrationTest : IntegrationTest() {
    ...
    fun `System returns http status 403 if certificate serial number is not attached to the request`() {
        // When the EMS send a delete request to "/postalvotes" with an application id APPLICATION_ID_1 and without the certificate serial number in the request header
        val responseSpec = apiClient!!.delete("/postalvotes/$APPLICATION_ID_1", attachSerialNumber = false)
