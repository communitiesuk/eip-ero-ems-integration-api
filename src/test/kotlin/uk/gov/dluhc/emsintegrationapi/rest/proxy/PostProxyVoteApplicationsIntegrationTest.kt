...
class PostProxyVoteApplicationsIntegrationTest : IntegrationTest() {
    ...
    fun `System returns http status 403 if certificate serial number is not attached to the request`() {
        ...
        val responseSpec =
            apiClient!!.postEmsApplication("/proxyvotes/502cf250036469154b4f85fb", attachSerialNumber = false)
    ...
    fun `System rejects the request with status code 400 if the application id format is invalid`() {
        ...
        val responseSpec = apiClient!!.postEmsApplication("/proxyvotes/123", serialNumber = "1234567891")
    ...
    fun `System returns http status 404 if a given application does not exist`() {
        // When the EMS sends a post request to "/proxyvotes" with an application id "502cf250036469154b4f85aa" and certificate serial number "1234567891" and SUCCESS status
        val responseSpec =
            apiClient!!.postEmsApplication("/proxyvotes/502cf250036469154b4f85aa", serialNumber = "1234567891")
        ...
        val message = responseSpec.returnResult(String::class.java).responseBody.blockFirst()
        assertThat(message).isEqualTo("The Proxy application could not be found with id `502cf250036469154b4f85aa`")
    }
    ...
    fun `System returns http status 404 if the gss codes retrieved from ERO and the application gss code are different`() {
        ...
        // When the EMS sends a post request to "/proxyvotes" with an application id "502cf250036469154b4f85fc" and certificate serial number "1234567891" and SUCCESS status
        val responseSpec =
            apiClient!!.postEmsApplication("/proxyvotes/502cf250036469154b4f85fc", serialNumber = "1234567891")
    ...
    fun `System returns http status 204 on successful deletion of a success status`() {
        ...
        // When the EMS sends a post request to "/proxyvotes" with an application id "502cf250036469154b4f85fb" and certificate serial number "1234567891" and SUCCESS status
        val responseSpec =
            apiClient!!.postEmsApplication("/proxyvotes/502cf250036469154b4f85fb", serialNumber = "1234567891")

        // Then the system updated the proxy application with the id "502cf250036469154b4f85fb" status as "DELETED"
        val proxyVoteApplication = proxyApplicationRepository.findById("502cf250036469154b4f85fb").get()
    ...
    fun `System returns http status 204 on successful deletion of a failure status`() {
        ...
        // When the EMS sends a post request to "/proxyvotes" with an application id "502cf250036469154b4f85fb" and certificate serial number "1234567891" and FAILURE status
        val responseSpec =
            apiClient!!.postEmsApplication(
                "/proxyvotes/502cf250036469154b4f85fb",
    ...
    fun `System ignores the request if proxy vote application is already DELETED and no message will be place on queue`() {
        ...
        val applicationFromDB = proxyApplicationRepository.findById("502cf250036469154b4f85fb").get()
