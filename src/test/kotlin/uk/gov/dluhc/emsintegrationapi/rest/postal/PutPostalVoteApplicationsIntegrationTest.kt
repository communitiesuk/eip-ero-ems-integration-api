...
internal class PutPostalVoteApplicationsIntegrationTest : IntegrationTest() {
    @Autowired
    ...
    fun `System returns http status 403 if certificate serial number is not attached to the request`() {
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_1 and without the certificate serial number in the request header
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_1", attachSerialNumber = false)
    ...
    fun `System rejects the request with status code 400 if the application id format is invalid`() {
        ...
        val responseSpec = apiClient!!.putEmsApplication("/postalvotes/123", serialNumber = ERO_ID_1_CERTIFICATE_SERIAL)
    ...
    fun `System returns http status 404 if a given application does not exist`() {
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_3 and certificate serial number ERO_ID_1_CERTIFICATE_SERIAL and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_1", serialNumber = ERO_ID_1_CERTIFICATE_SERIAL)
        ...
        val errorResponse = responseSpec.returnResult(ErrorResponse::class.java).responseBody.blockFirst()
        assertThat(errorResponse!!.message).isEqualTo("The Postal application could not be found with id `$APPLICATION_ID_1`")
    }
    ...
    fun `System returns http status 404 if the gss codes retrieved from ERO and the application gss code are different`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_2 and certificate serial number ERO_ID_1_CERTIFICATE_SERIAL and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_2", serialNumber = ERO_ID_1_CERTIFICATE_SERIAL)
        ...
        val errorResponse = responseSpec.returnResult(ErrorResponse::class.java).responseBody.blockFirst()
        assertThat(errorResponse!!.message).isEqualTo("The Postal application could not be found with id `$APPLICATION_ID_2`")
    }
    ...
    fun `System returns http status 204 on successful deletion of a success status`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_1 and certificate serial number ERO_ID_1_CERTIFICATE_SERIAL and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_1", serialNumber = ERO_ID_1_CERTIFICATE_SERIAL)
    ...
    fun `System returns http status 204 on successful deletion of a failure status`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_1 and certificate serial number ERO_ID_1_CERTIFICATE_SERIAL and FAILURE status
        val responseSpec =
