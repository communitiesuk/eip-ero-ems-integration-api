...
internal class PutPostalVoteApplicationsIntegrationTest : IntegrationTest() {
    ...
    fun `System rejects the request with status code 400 if the application id format is invalid`() {
        ...
        val responseSpec = apiClient!!.putEmsApplication("/postalvotes/123", serialNumber = CERTIFICATE_SERIAL_NUM_1)
    ...
    fun `System returns http status 404 if a given application does not exist`() {
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_3 and certificate serial number CERTIFICATE_SERIAL_NUM_1 and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_1", serialNumber = CERTIFICATE_SERIAL_NUM_1)
    ...
    fun `System returns http status 404 if the gss codes retrieved from ERO and the application gss code are different`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_2 and certificate serial number CERTIFICATE_SERIAL_NUM_1 and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_2", serialNumber = CERTIFICATE_SERIAL_NUM_1)
    ...
    fun `System returns http status 204 on successful deletion of a success status`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_1 and certificate serial number CERTIFICATE_SERIAL_NUM_1 and SUCCESS status
        val responseSpec =
            apiClient!!.putEmsApplication("/postalvotes/$APPLICATION_ID_1", serialNumber = CERTIFICATE_SERIAL_NUM_1)
    ...
    fun `System returns http status 204 on successful deletion of a failure status`() {
        ...
        // When the EMS sends a put request to "/postalvotes" with an application id APPLICATION_ID_1 and certificate serial number CERTIFICATE_SERIAL_NUM_1 and FAILURE status
        val responseSpec =
