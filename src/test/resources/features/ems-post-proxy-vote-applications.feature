...
Feature: The EMS send a post request on confirming a given Proxy Vote Application by Id (Soft deletion)
  ...
  Scenario: System returns http status 403 if certificate serial number is not attached to the request
    When the EMS sends a post request to "/proxyvotes" with an application id "502cf250036469154b4f85fb" and without the certificate serial number in the request header
